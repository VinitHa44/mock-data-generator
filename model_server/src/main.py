            ...
            with self._lock:
                if not hasattr(self, "initialized"):
                    ...
                    self.text_moderation_pipeline = pipeline(
                        "text-classification", model="KoalaAI/Text-Moderation"
                    )
...
async def validate_harmful(payload: TextPayload):
    results = moderation_service.text_moderation_pipeline(
        payload.text, top_k=None
    )
...
async def validate_injection(payload: TextPayload):
    ...
    if (
        result
        and result[0]["label"] == "INJECTION"
        and result[0]["score"] > 0.8
    ):
