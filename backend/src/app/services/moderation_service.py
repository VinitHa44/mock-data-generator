...
class ModerationService:
            ...
            with self._lock:
                if not hasattr(self, "initialized"):
                    ...
                    self.client = httpx.AsyncClient(
                        base_url=settings.MODEL_SERVER_URL, timeout=30.0
                    )
    ...
    async def _is_harmful_content(self, text: str) -> bool:
        ...
        try:
            response = await self.client.post(
                "/validate_harmful", json={"text": text}
            )
            ...
            logger.error(
                "Error calling model server for harmful content check.",
                exc_info=True,
            )
            return True  # Fail safe, assume harmful
        except Exception:
            logger.error(
                "An unexpected error occurred during harmful content check.",
                exc_info=True,
            )
    ...
    async def _is_prompt_injection(self, text: str) -> bool:
        ...
        try:
            response = await self.client.post(
                "/validate_injection", json={"text": text}
            )
            response.raise_for_status()
            result = response.json()
            ...
            logger.error(
                "Error calling model server for prompt injection check.",
                exc_info=True,
            )
            return True  # Fail safe, assume injection
        except Exception:
            logger.error(
                "An unexpected error occurred during prompt injection check.",
                exc_info=True,
            )
            return True  # Fail safe
        return False
    ...
    async def validate_input(self, payload: list) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to extract string values from payload for validation.",
                exc_info=True,
            )
