...
class CacheService:
    ...
    async def find_group_by_hashes(self, object_hashes: List[str]) -> Optional[str]:
        """
        ...
        try:
            ...
            for group_key in all_group_keys:
                ...
                stored_hashes = await self.redis_client.lrange(f"{group_key}:hashes", 0, -1)
                if any(h in stored_hashes for h in object_hashes):
                    logger.info("Found matching cache group", group_id=group_key)
                    return group_key.split(":")[1]
            return None
        except redis.exceptions.RedisError as e:
            logger.error("Redis error while finding group by hashes", error=str(e))
            raise CacheServiceError("Failed to search cache for matching groups.")

    async def get_data_if_cache_hit(self, object_hashes: List[str], required_count: int) -> Optional[List[Dict[str, Any]]]:
        """
        ...
        try:
            ...
            for group_hash_key in all_group_keys:
                stored_hashes = await self.redis_client.lrange(group_hash_key, 0, -1)
                if any(h in stored_hashes for h in object_hashes):
    ...
    async def get_cached_mock_data(self, group_id: str) -> Optional[List[Dict[str, Any]]]:
        """
    ...
    async def update_group_hashes(self, group_id: str, new_hashes: List[str]):
        ...
        try:
            ...
            logger.error("Redis error while updating group hashes", error=str(e))
            # This is a non-critical error, so we just log it.
    
    async def create_new_group_cache(self, object_hashes: List[str], mock_data: List[Dict[str, Any]]) -> str:
        """
        ...
        try:
            ...
            logger.error("Redis error while creating new cache group", error=str(e))
            raise CacheServiceError("Failed to create a new group in the cache.")
