...
class ImageEnrichmentService:
    ...
    def _extract_keywords_from_url(self, url: str) -> str:
        ...
        try:
            ...
            meaningful_dirs = [
                d for d in potential_keywords if d.lower() not in generic_dirs
            ]
    ...
    async def _fetch_image_url_from_openverse(self, keywords: str) -> str:
        ...
        try:
            ...
            return (
                "https://via.placeholder.com/150"  # Return placeholder on error
            )
    ...
    async def _traverse_and_enrich(
        ...
        if isinstance(data, dict):
            return {
                key: await self._traverse_and_enrich(value, parent_key=key)
                for key, value in data.items()
            }
        elif isinstance(data, list):
            return [
                await self._traverse_and_enrich(item, parent_key=parent_key)
                for item in data
            ]
