...
from app.utils.app_exceptions import InsufficientDataError, LLMGenerationError
...
class GenerateMockDataUsecase:
    ...
    def _save_intermediate_step(self, data: Any, file_prefix: str, request_id: str):
        ...
        path = f"{settings.INTERMEDIATE_SAVE_PATH}/{file_prefix}_{request_id}.json"
        try:
    ...
    async def _generate_and_validate_data(self, input_examples: List[Dict], count: int, request_id: str) -> List[Dict]:
        """
        ...
        while len(total_generated_data) < count and attempts < max_attempts:
            ...
            generated_data = await llm_service.generate_mock_data(input_examples, remaining_count)
            self._save_intermediate_step(generated_data, f"llm_response_attempt_{attempts+1}", request_id)

            if not generated_data:
                logger.warning("LLM returned no data on this attempt.", attempt=attempts+1)
                attempts += 1
                continue
            ...
            if is_copy:
                logger.warning("LLM returned a copy of the input examples. Discarding and retrying.", attempt=attempts+1)
                attempts += 1
                continue
        ...
        if len(total_generated_data) < count:
            logger.error("LLM failed to generate the required number of items.", required=count, generated=len(total_generated_data), attempts=max_attempts)
            raise LLMGenerationError(f"LLM failed to generate the required number of items after {max_attempts} attempts.")
        
        # Ensure we don't return more than requested
        return total_generated_data[:count]

    async def execute(self, input_examples: List[Dict], count: int) -> Tuple[Dict, bool]:
        request_id = str(uuid.uuid4())
        logger.info("Starting mock data generation use case", request_id=request_id, requested_count=count)
        
        # 1. Check cache using the new efficient method
        object_hashes = compute_hashes_for_payload(input_examples)
        cached_data = await cache_service.get_data_if_cache_hit(object_hashes, count)
        
        if cached_data:
            logger.info("Full response served from cache.", request_id=request_id)
            return (cached_data[:count], True)

        # 2. Generate Data if cache missed
        logger.info("Cache miss or insufficient data. Generating new mock data.", request_id=request_id)
        ...
        await log_repository.log_token_usage(request_id, input_tokens, output_tokens, "LocalSmolLM")
